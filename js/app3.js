    ...
    $.alert({
        ...
        buttons: {
            login: {
                text: 'تسجيل الدخول',
                btnClass: 'btn-blue',
                action: function () { window.location.href = HomeURL + "/login/"; }
            },
            register: {
                text: 'تسجيل جديد',
                btnClass: 'btn-green',
                action: function () { window.location.href = HomeURL + "/register/"; }
            }
        }
    });
}
...
$("body").on('click', '.FollowSeries', function(event) {
    if (isUser == "false")
    {
        notUserMsg();
    }
    else
    {
        $.ajax({
            url: MyAjaxURL+'Users/MetaAction.php',
            type: 'POST',
            data: {post_id: $(this).data("post") , "meta": "Followselary" },
            success:function(requestResponse){
                $("body").append(requestResponse)
            }
        });
